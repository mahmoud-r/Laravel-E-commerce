@extends('admin.master')


@section('breadcrumb')
    <li class="breadcrumb-item "><a href="{{route('dashboard')}}">Dashboard</a></li>
    <li class="breadcrumb-item "><a href="{{route('reviews.index')}}">Brands</a></li>
    <li class="breadcrumb-item active">list</li>
@endsection
@section('header')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><a style="color: black" href="{{route('reviews.index')}}">Reviews</a></h1>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
    </section>
@endsection

@section('style')
    <style>

    </style>
@endsection

@section('content')
    <div class="container-fluid">
        <div class="card">
            <form action="" method="get">

            <div class="card-header">
                <div class="card-title">
                        <button type="button" onclick="window.location.href='{{route('reviews.index')}}'" class="btn btn-default btn-sm">Reset</button>
                </div>
                <div class="card-tools">
                    <div class="input-group input-group" style="width: 250px;">
                        <input type="text" name="keyword" value="{{Request::get('keyword')}}" class="form-control  float-right" placeholder="Search">

                        <div class="input-group-append">
                            <button type="submit" class="btn btn-default">
                                <i class="fas fa-search"></i>
                            </button>
                        </div>
                    </div>
                </div>
            </div>

            </form>

            <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                    <thead>
                    <tr>
                        <th width="60">#</th>
                        <th >Product</th>
                        <th>User</th>
                        <th >Star</th>
                        <th>Comment</th>
                        <th >Status</th>
                        <th >Date</th>
                        <th>Publish</th>
                        <th >Delete</th>
                    </tr>
                    </thead>
                    <tbody>
                        @forelse($reviews as $i=> $review)

                            <tr id="review_{{ $review->id }}">
                                <td>{{ $reviews->firstItem() + $i }}</td>
                                <td>
                                    <a href="{{route('products.edit',$review->product->id)}}" class="d-flex gap-2 align-items-center">

                                        @if(!empty($review->product->images->first()->image))
                                            <img src="{{asset('uploads/products/images/thumb/'.$review->product->images->first()->image)}}" class="rounded" width="38" loading="lazy">
                                        @else
                                            <img  src="{{asset('front_assets/images/empty-img.png')}}" class="rounded"  width="38" loading="lazy">

                                        @endif
                                        {{$review->product->title}}
                                    </a>
                                <td><a href="{{route('users.edit',$review->user->id)}}">{{$review->user->name}}</a></td>
                                <td>
                                    <div class="rating_wrap">
                                        <div class="rating">
                                            <div class="product_rate" style="width:{{$review->rating_percentage}}%"></div>
                                        </div>
                                    </div>
                                </td>
                                <td style="white-space: normal;">{{$review->comment}}</td>

                                <td>
                                    @if($review->status == 1 )

                                    <svg class="text-success-500 h-6 w-6 text-success" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    @else
                                        <svg class="text-danger h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                    @endif
                                </td>
                                <td>{{\carbon\Carbon::parse($review->created_at)->format('d M,Y')}}</td>
                                <td>
                                    <form method="post" action="{{route('reviews.publishToggle')}}">
                                        @csrf
                                        <input type="hidden" name="review_id" value="{{$review->id}}">
                                        <input type="hidden" name="status" value="{{$review->status == 0 ?1 : 0}}">
                                        <button class="btn btn-info  btn-trigger-confirm-payment   {{$review->status == 0 ?'save-btn' : 'btn-info'}} " type="submit" >
                                            {{$review->status == 0 ?'Publish' : 'UnPublish'}}
                                        </button>
                                    </form>

                                </td>

                                <td>
                                    <a href="#" onclick="ReviewDelete({{$review->id}},'{{$review->user->name}}')" class="text-danger w-4 h-4 mr-1">
                                        <svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                            <path	ath fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                        </svg>
                                    </a>
                                </td>
                            </tr>

                        @empty
                        <tr>
                            <td colspan="9" class="text-center"> Currently, there are no reviews.</td>
                        </tr>
                        @endforelse



                    </tbody>
                </table>
            </div>
            <div class="card-footer clearfix">
                {{$reviews->links()}}
            </div>
        </div>
    </div>

@endsection







@section('script')

<script>

    function ReviewDelete(id,name) {
        Swal.fire({
            title: "Do you want to Delete "+name+" review ?",
            showCancelButton: true,
            confirmButtonText: "Delete",
            confirmButtonColor: "#dc3545",
        }).then((result) => {
            if (result.isConfirmed) {
                var url = '{{Route("reviews.destroy","ID")}}';
                var newUrl =url.replace('ID',id)
                $.ajax({
                    url: newUrl,
                    type: 'DELETE',
                    data: '',
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')
                    },success: function (response) {

                        if(response['status'] === true){
                            Swal.fire("Deleted!", "", "success");
                            $('#review_' + id).remove();
                        }else {
                            console.log('something went wrong')
                        }
                    }

                });
            }
        });
    }

</script>
@endsection
